...
def increment(sequence):
    return format((int(sequence, 2) + 1) % 4096, '012b')
# --------------------------------------------
...
ciphered_image = []
for part in splitted_binary_picture:
    ciphered_counter = mini_des(increment(counter), input_key, input_permutation, sbox_1, sbox_2)
    ciphered_image.append(find_xor(part, ciphered_counter))

ciphered_image = "".join(ciphered_image)

ciphered_image = fill_padding(ciphered_image, width, height)
write_picture('ciphed_CTR.pbm', width, height, ciphered_image)
